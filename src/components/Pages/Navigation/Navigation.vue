...
#Nav.card
  ...
  .card-body
    .card.add
      ...
      .card-body
        .input-group
          input.form-control(type='text', placeholder="請輸入標題", aria-label="請輸入標題", aria-describedby='basic-addon2', v-model="newTitle")
          .input-group-append
            button.btn.btn-outline-success(type='button', @click="addNewNavigation") 新增
    ul.list-group
        ...
        .top
          ...
          .actions
            .btn-group(role='group', aria-label='actions')
              ...
              button.btn.btn-danger(type='button', data-toggle="modal" data-target="#delete-nav", @click="deleteNavigation(navItem.navigation_id)") 刪除
...
export default {
  ...
  methods: {
    ...
    addNewNavigation () {
      const self = this
      api.navigation.create({name: this.newTitle}).then(response => {
        self.initData()
      })
    },
    deleteNavigation (navigationId) {
      const self = this
      api.navigation.delete({navigationId: navigationId}).then(response => {
        self.initData()
      })
    }
  }
